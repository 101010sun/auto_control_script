...
class ContactServer:
    ...
    def run(self):
        ...
        act_samsung_scenario = threading.Thread(target=samsungS20FE.process_scenario, args=(scenario_value,))
        act_samsung_scenario.start()
        ## oppo cellphone
        scenario_value = self.get_random_scenario()
        act_oppo_scenario = threading.Thread(target=oppoReno7.process_scenario, args=(scenario_value,))
        ...
        ## acer laptop
        scenario_value = self.get_random_scenario()
        conn_acer_scenario = threading.Thread(target=self.connect_to_client, args=(self.acer_host, self.acer_port, 'normal_flow', scenario_value,))
        conn_acer_scenario.start()
        ## asus laptop
        scenario_value = self.get_random_scenario()
        conn_asus_scenario = threading.Thread(target=self.connect_to_client, args=(self.asus_host, self.asus_port, 'normal_flow', scenario_value,))
        conn_asus_scenario.start()
        ## msi laptop
        scenario_value = self.get_random_scenario()
        conn_msi_scenario = threading.Thread(target=self.connect_to_client, args=(self.msi_host, self.msi_port, 'normal_flow', scenario_value,))
        ...
        conn_acer_attack = threading.Thread(target=self.connect_to_client, args=(self.acer_host, self.acer_port, 'attack_start',))
        conn_acer_attack.start()
        
        conn_asus_attack = threading.Thread(target=self.connect_to_client, args=(self.asus_host, self.asus_port, 'attack_start',))
        conn_asus_attack.start()
        
        conn_msi_attack = threading.Thread(target=self.connect_to_client, args=(self.msi_host, self.msi_port, 'attack_start',))
